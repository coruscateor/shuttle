...
pub trait ConfigManager: Sized {
        ...
        C: Serialize
    {
        ...
        config_file
            ...
            .with_context(|| anyhow!("Could not write the global configuration file: {}", path.display()))?;
        Ok(())
    }
}
...
impl ConfigManager for GlobalConfigManager {
    fn directory(&self) -> PathBuf {
        let shuttle_config_dir = dirs::config_dir().ok_or_else(|| anyhow!(
...
fn find_crate_name<P: AsRef<Path>>(working_directory: P) -> Result<ProjectName> {
    ...
    let package_name = meta
        .root_package()
        .ok_or_else(|| anyhow!("could not find a root package in `{}`", working_directory.as_ref().display()))?
        .name
        .clone()
        .parse()?;
    Ok(package_name)
...
impl RequestContext {
    ...
    pub fn load_global() -> Result<Self> {
        ...
        if ! global.exists() { global.create()?; }
        global.open()?;
        Ok(Self { global, project: None })
    }
    ...
    pub fn api_key(&self) -> Result<&ApiKey> {
        self.global
            ...
            .api_key()
            .ok_or_else(|| {
                anyhow!("Configuration file: `{}`", self.global.manager.path().display())
    ...
    pub fn working_directory(&self) -> &Path {
        self.project.as_ref().unwrap().manager.working_directory.as_path()
    }
