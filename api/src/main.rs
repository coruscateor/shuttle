...
#[post("/users/<username>")]
async fn get_or_create_user(user_directory: &State<UserDirectory>, username: String, _admin: Admin) -> Result<ApiKey, AuthorizationError> {
...
async fn create_project(
    ...
    if user.projects.iter().find(|my_project| **my_project == project_name).is_none() {
...
async fn rocket() -> Rocket<Build> {
    ...
    let user_directory = UserDirectory::from_user_file()
        .expect("could not initialise user directory");
