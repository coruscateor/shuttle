    ...
    async fn from_request(request: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        if let Some(authorization) = request.headers().get_one("Authorization") {
            match ApiKey::from_authorization_header(authorization) {
                Ok(api_key) => {
                    let authorizer: &'r State<UserDirectory> = try_outcome!(
                        request.guard().await.map_failure(|(status, ())| {
...
impl<'r> FromRequest<'r> for ScopedUser {
    ...
    async fn from_request(request: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        ...
        let route = request.route().expect("`User` can only be used in requests");
        if route.uri.base().starts_with("/projects") {
            match request.param::<ProjectName>(0) {
                ...
                Some(Err(_)) => Outcome::Failure((Status::NotFound, AuthorizationError::NotFound(()))),
                None => Outcome::Failure((Status::Unauthorized, AuthorizationError::Unauthorized(())))
            }
        } else {
...
impl UserDirectory {
    ...
    pub(crate) fn create_project_if_not_exists(&self, username: &str, project_name: &ProjectName) -> Result<(), DeploymentApiError> {
        {
    ...
    pub(crate) fn authorize(&self, key: &ApiKey, project_name: &ProjectName) -> Option<User> {
        let user = self.users.read().unwrap().get(&key).cloned()?;
